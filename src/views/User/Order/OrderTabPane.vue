...
export default defineComponent({
  ...
  setup({status}) {
    ...
    const getOrders = async (fetchData: GetOrders) => {
      ...
      loading.value = true
      const {data} = await orderApi.getOrders({size: paginationOptions.size, ...getDateRange(currentOrderTimeRange.value.key), ...fetchData, status: status === 999 ? undefined : status}).finally(() => loading.value = false)
